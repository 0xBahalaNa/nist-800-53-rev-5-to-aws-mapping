...
I map NIST 800-53 Rev 5 security controls to the AWS services that support their implementation, stored as an OSCAL Component Definition JSON file. A Python generator renders that mapping as markdown, with optional FedRAMP High baseline filtering and a CJIS v6.0 delta section that calls out where CJIS exceeds FedRAMP requirements. Thirty-one controls in the component definition; five of them carry a `cjis-delta` prop.
